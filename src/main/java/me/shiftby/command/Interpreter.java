...
public class Interpreter {
    ...
    public Command interpret(String command, User sender) {
        ...
        } else if (command.startsWith("/gmembers ")) {
            Pattern pattern = Pattern.compile("/gmembers\\s+(.*)");
            Matcher matcher = pattern.matcher(command);
            if (matcher.matches() && matcher.groupCount() == 1) {
                return new GroupMembersCommand(sender, matcher.group(1));
            } else {
                return new InvalidArgumentsCommand(sender);
            }
        } else if (command.startsWith("/ugroups ")) {
            Pattern pattern = Pattern.compile("/ugroups\\s+(.*)");
            Matcher matcher = pattern.matcher(command);
            if (matcher.matches() && matcher.groupCount() == 1) {
                return new UserGroupsCommand(sender, matcher.group(1));
            } else {
                return new InvalidArgumentsCommand(sender);
            }
        } else if (command.equals("/gshow")) {
            return new ShowGroupsCommand(sender);
        } else if (command.equals("/ushow")) {
            return new ShowUsersCommand(sender);
